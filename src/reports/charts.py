...
class ChartGenerator:
    ...
    COUNTRY_COLORS = [
        '#1a476f', '#e37222', '#2f7e44', '#c42c2c', '#6e4c9e',
        '#d4a100', '#17becf', '#e377c2', '#8c564b', '#7f7f7f',
        '#bcbd22', '#ff7f0e', '#2ca02c', '#d62728', '#9467bd',
        '#1f77b4', '#98df8a', '#c5b0d5', '#c49c94', '#f7b6d2',
    ]
    ...
    def create_radar_chart(
        ...
        for i, (country, data) in enumerate(country_data.items()):
            ...
            max_vals = {m: max(country_data[c].get(m, 1) for c in country_data) for m in metrics}
            values_norm = [v / max_vals[m] * 100 if max_vals[m] > 0 else 0 for v, m in zip(values, metrics)]
            values_norm += values_norm[:1]

            color = self.COUNTRY_COLORS[i % len(self.COUNTRY_COLORS)]
            ax.plot(angles, values_norm, linewidth=2, linestyle='solid', label=country, color=color)
    ...
    def create_heatmap(
        ...
        # Create heatmap
        im = ax.imshow(data, cmap=cmap, aspect='auto', vmin=0, vmax=max(0.2, data.max()))
            ...
            for i in range(n):
                for j in range(n):
                    ...
                    if val > 0:
                        ...
                        ax.text(j, i, f'{val*100:{value_format}}%', ha='center', va='center',
    ...
    def create_war_probability_heatmap(
        ...
        matrix = {c: {c2: 0.0 for c2 in countries} for c in countries}
    ...
    def create_scatter_correlation(
        ...
        colors = [self.COUNTRY_COLORS[i % len(self.COUNTRY_COLORS)] for i in range(len(countries))]
        ...
        for i, country in enumerate(countries):
            ax.annotate(country, (x_values[i], y_values[i]),
        ...
        if len(x_values) > 2:
            ...
            ax.plot(x_line, p(x_line), '--', color=self.COLORS['neutral'],
    ...
    def create_bubble_chart(
        ...
        scatter = ax.scatter(x_values, y_values, s=sizes_norm, c=colors,
        ...
        # Add quadrant lines
        ax.axvline(x=50, color=self.COLORS['light'], linestyle='--', linewidth=1, alpha=0.7)
        ax.axhline(y=10, color=self.COLORS['light'], linestyle='--', linewidth=1, alpha=0.7)

        # Quadrant labels
        ax.text(75, 2, 'Free & Stable\n(Ideal)', ha='center', va='center',
               fontsize=10, color=self.COLORS['positive'], alpha=0.7)
        ax.text(25, 2, 'Unfree & Stable', ha='center', va='center',
               fontsize=10, color=self.COLORS['neutral'], alpha=0.7)
        ax.text(75, 25, 'Free & Inflationary', ha='center', va='center',
               fontsize=10, color=self.COLORS['highlight'], alpha=0.7)
        ax.text(25, 25, 'Unfree & Inflationary\n(Worst)', ha='center', va='center',
               fontsize=10, color=self.COLORS['negative'], alpha=0.7)
        ...
        ax.legend(loc='upper right', frameon=True, facecolor='white', edgecolor='none', title='GDP Size')
    ...
    def create_donut_chart(
        ...
        colors = [self.COUNTRY_COLORS[i % len(self.COUNTRY_COLORS)] for i in range(len(labels))]
    ...
    def create_damage_breakdown_donut(
        ...
        values = {
            'Central Bank': cb_damage,
            'Government': gov_damage
        }
    ...
    def create_stacked_area_evolution(
        ...
        n_months = len(list(country_metrics.values())[0]) if country_metrics else 0
    ...
    def create_histogram(
        ...
        ax.axvline(mean_val, color=self.COLORS['negative'], linestyle='--',
                  linewidth=2, label=f'Mean: {mean_val:.2f}')
        ax.axvline(median_val, color=self.COLORS['positive'], linestyle='-',
    ...
    def create_ranking_horizontal_bar(
        ...
        for i, (bar, label) in enumerate(zip(bars, labels)):
            ax.text(-max_val * 0.08, bar.get_y() + bar.get_height()/2,
    ...
    def create_regime_comparison_chart(
        ...
        for idx, (metric, label) in enumerate(zip(metrics, metric_labels)):
            ...
            for regime in regimes:
                ...
                values = [v * 100 if v < 1 and metric != 'freedom_index' else v for v in values]
                avg_values.append(np.mean(values) if values else 0)

            # Sort by value
            sorted_data = sorted(zip(regimes, avg_values),
                               key=lambda x: x[1],
                               reverse=(metric == 'freedom_index'))
            regimes, avg_values = zip(*sorted_data) if sorted_data else ([], [])
            ...
            bars = ax.barh(y_pos, avg_values, color=colors, alpha=0.85,
                          edgecolor='white', linewidth=0.5)
